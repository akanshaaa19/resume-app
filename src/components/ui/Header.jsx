...
function Header() {
  ...
  return (
    <header className=" px-8 fixed z-50 text-xl bg-[#1e1e1e] font-semibold flex items-center justify-between w-full">
      <div className="flex items-center">
        {isLoggedIn && <div className="md:hidden flex mr-8">
          {!showSideBar && (
            <i
              onClick={() => dispatch(uiAction.showSideBar(true))}
              class="fa-solid fa-bars "
            ></i>
          )}
          {showSideBar && (
            <i
              onClick={() => dispatch(uiAction.showSideBar(false))}
